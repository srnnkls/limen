...
| `UserPromptSubmit` | `prompt` | prompt context, `limen-herd`, `limen-inbox` |
| `PreToolUse` | `tool_name`, `tool_input`, `tool_use_id` | `limen-inbox` |
| `PostToolUse` | `tool_name`, `tool_input` (with `file_path` for an edit), `tool_use_id` | edit review, `limen-inbox` |
| `Stop` | `stop_hook_active`, `last_assistant_message`, `context_tokens`, `context_window` | `limen-usage`, `limen-inbox`, `limen-herd` |
| `PostModelSwitch` | `to_model`, `context_tokens`, `context_window` | `limen-model`, `limen-usage` |
| `SessionEnd` | `reason` | `limen-inbox`, `limen-herd` |
