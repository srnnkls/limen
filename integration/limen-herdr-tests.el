...
(ert-deftest limen-herdr-claude-adopt-submits-ide-when-connecting ()
  (let* ((root (file-truename (make-temp-file "limen-herdr-adopt-ide" t)))
         (session (limen-herdr-tests--session "claude" root))
         (agent `((agent . "claude") (agent_status . "idle")
                  (server_key . "/tmp/herdr.sock")))
         (target (cons (herdr-agent-session-server session)
                       (herdr-agent-session-terminal session)))
         sent)
    (unwind-protect
        (cl-letf (((symbol-function 'herdr-agent-adopt)
                   (lambda (_agent &rest _) session))
                  ((symbol-function 'herdr-agent-send-text)
                   (lambda (target text) (push (list 'text target text) sent)))
                  ((symbol-function 'herdr-agent-send-keys)
                   (lambda (target keys) (push (list 'keys target keys) sent)))
                  ((symbol-function 'sleep-for) #'ignore))
          (let ((limen-herdr-claude-connect-on-adopt 'idle))
            (limen-herdr-claude-adopt agent)
            (should (equal (nreverse sent)
                           `((text ,target "/ide") (keys ,target ("return"))))))
          (setq sent nil)
          (let ((limen-herdr-claude-connect-on-adopt nil))
            (limen-herdr-claude-adopt agent)
            (should-not sent)))
      (delete-directory root t))))
