  ...
  (let ((limen-hooks--subscriptions nil)
    ...
    (cl-letf (((symbol-function 'limen-hooks-request-install)
              ...
              ((symbol-function 'limen-hooks--remove-groups)
               (lambda (provider predicate)
                 (push (cons provider
                             (seq-filter (lambda (event) (funcall predicate event nil))
                                         '("PostModelSwitch" "Stop" "SessionEnd")))
                       removed))))
      (limen-model-mode 1)
      (should (equal requested '("model")))
      (should (equal (limen-hooks-events 'claude) '(("PostModelSwitch") ("Stop"))))
      (should-not (limen-hooks-events 'codex))
      (should (memq #'limen-model--report-event limen-hooks-event-functions))
      (limen-model-mode 1)
      (should (equal (limen-hooks-events 'claude) '(("PostModelSwitch") ("Stop"))))
      (should (= (length limen-hooks--subscriptions) 1))
      (limen-model-mode -1)
