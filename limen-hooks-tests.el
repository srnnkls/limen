...
(defun limen-hooks-tests--tool-request (provider event tool file root)
  `((version . 1) (method . "hook") (provider . ,provider)
  ...
  (let* ((root (file-truename (make-temp-file "limen-hooks-review" t)))
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'run-at-time)
          ...
          (cl-flet ((fire (provider event tool file)
                      (limen-hooks-output
                       (limen-hooks-tests--tool-request provider event tool file root)
                       (limen-hooks-tests--request root))))
            ...
            (let ((limen-hooks-review-edits t))
              (dolist (tool '("Edit" "Write" "MultiEdit"))
                (should (equal (fire "claude" "PostToolUse" tool inside) "")))
              (should (equal reviewed (list inside inside inside)))
              (setq reviewed nil)
              (fire "claude" "PostToolUse" "Bash" inside)
              (fire "claude" "PreToolUse" "Edit" inside)
              (fire "claude" "PostToolUse" "Edit" away)
              (fire "codex" "PostToolUse" "Edit" inside)
              (should-not reviewed))))
      (delete-directory root t)
